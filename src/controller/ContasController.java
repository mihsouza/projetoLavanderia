...
public class ContasController extends BD {
	...
	public void inserirSalario() {
		...
		if (cbDivisao.getSelectedItem().toString().equals("Sim")) {
			...
			try {
				...
				frame.lblAviso.setText("Lançamento realizado no financeiro!");
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
		} else {
	...
	public void visualizar() {
		if (getConnection()) {
			try {
				String sql = "SELECT * FROM HISTCONTAS WHERE ID = " + tfId.getText();
				st = con.createStatement();
				rs = st.executeQuery(sql);
				while (rs.next()) {
	...
	public void excluir() {
		if (getConnection()) {
			try {
				...
				frame.setVisible(true);
				frame.setLocationRelativeTo(null);
				frame.lblAviso.setText("Lançamento excluído do financeiro!");
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
		}
	...
	public ContasController(JRadioButton rbNaoPago, JRadioButton rbPago, JTextField tfPesquisa, JTextField tfInicio,
			JTextField tfFim, DefaultTableModel modelo) {
		this.rbNaoPago = rbNaoPago;
		this.rbPago = rbPago;
	...
	public void listar() {
		...
		if (rbNaoPago.isSelected() && rbPago.isSelected()) {
			sql = "SELECT ID, PAGOA, VALOR, DATA, STATUS " + "FROM HISTCONTAS WHERE DATA BETWEEN '" + tfInicio.getText()
		...
		if (getConnection()) {
			try {
				...
				while (rs.next()) {
					modelo.addRow(new Object[] {rs.getInt("ID"), rs.getString("PAGOA"), "R$ " + rs.getDouble("VALOR") + "0", rs.getString("DATA"), rs.getString("STATUS")});
				}
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
