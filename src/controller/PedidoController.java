...
public class PedidoController {
		...
		if (b.getConnection()) {
			...
			try {
				...
				while (b.rs.next()) {
					...
					frame.setVisible(true);
					frame.setLocationRelativeTo(null);
					frame.lblAviso.setText("Pedido gerado com sucesso!");
				}
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
	...
	public void consulta() {
		...
		if (rbAtraso.isSelected()) {
			status = "Em atraso";
			sql = "SELECT P.ID, C.CLIENTE, F.FUNCIONARIO, P.STATUS, P.PRECO "
					+ "FROM PEDIDO P, CLIENTE C, FUNCIONARIO F " + "WHERE P.CLIENTE = C.ID "
		...
		if (b.getConnection()) {
			try {
				...
				while (b.rs.next()) {
					modelo.addRow(new Object[] {b.rs.getInt("ID"), b.rs.getString("CLIENTE"), b.rs.getString("FUNCIONARIO"), b.rs.getString("STATUS"), "R$ " + b.rs.getDouble("PRECO") + "0"});
				}
			} catch (SQLException e) {
				System.out.println("ERRO" + e.toString());
			}
		...
		public void receber() {
			...
			if (b.getConnection()) {
				try {
					...
					frame.setVisible(true);
					frame.setLocationRelativeTo(null);
					frame.lblAviso.setText("Recebido!");
				} catch (Exception erro) {
					System.out.println("ERRO" + erro.toString());
				}
			} else {
