...
public class OrdemServicoController {
	...
	public void editar() {
		...
		if (b.getConnection()) {
			try {
				...
				JOptionPane.showMessageDialog(null, "OS nº : " + o.getId() + " alterada com sucesso!");
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
		} else {
	...
	public void visualizar() {
		int id = Integer.parseInt(JOptionPane.showInputDialog("Numero da OS que deseja visualizar: "));
		services.BD b = new services.BD();
		if (b.getConnection()) {
			try {
				String sql = "SELECT O.ID, C.CLIENTE, O.PEDIDO, F.FUNCIONARIO, O.DATAENTRADA, O.DATAPREVISTA, O.STATUS, O.PRECO, O.DATAEXECUT, O.OBSERVACAO "
						...
						+ "AND P.ID = O.PEDIDO " + "AND P.CLIENTE = C.ID " + "AND O.ID = " + id;
				b.st = b.con.createStatement();
				b.rs = b.st.executeQuery(sql);
				while (b.rs.next()) {
					tfId.setText(String.valueOf(b.rs.getInt("ID")));
	...
	public void excluir() {
		...
		if (b.getConnection()) {
			...
			try {
				b.st = b.con.createStatement();
				b.st.executeUpdate(sql);
				JOptionPane.showMessageDialog(null, "OS nº : " + o.getId() + "\nexcluido com sucesso!");
			} catch (Exception erro) {
				System.out.println("ERRO" + erro.toString());
			}
		} else {
		...
		if (b.getConnection()) {
			try {
				...
				while (b.rs.next()) {
					...
					sb.append(" ");
					sb.append("ID PEDIDO: " + b.rs.getString("PEDIDO"));
					sb.append(" ");
					sb.append("FUNCIONARIO: " + b.rs.getString("FUNCIONARIO"));
					sb.append(" ");
					sb.append("PREÇO: R$" + b.rs.getString("PRECO"));
					sb.append(" ");
					sb.append("DATA EXECUÇÃO: " + b.rs.getString("DATASAIDA"));
					sb.append(" ");
